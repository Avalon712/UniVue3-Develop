...
namespace UniVue.UI
{
    public sealed class RedPointMgr
    {
        ...
        {
            YieldWaitForEndOfFrame endOfFrame = new();
            while (true)
            {
                yield return endOfFrame;
                foreach (ulong dirtyTree in _dirtyTrees)
                    if (_trees.TryGetValue(dirtyTree, out RedPointNode root))
        ...
        public bool GetStatus(ulong key)
        {
